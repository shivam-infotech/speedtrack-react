...
const ActionCell = ({ icon, title, onClick, href, disabled }) => {
  ...
  let cell = (
    <Box onClick={!disabled && onClick} className={`${classes.actionCell} ${disabled ? classes.disabledAction : ''}`}>
      <div>{icon}</div>
      <Typography fontSize="0.65rem" color="textSecondary">{title}</Typography>
    </Box>
  );
  if (href && !disabled) cell = <a href={href} target="_blank" rel="noreferrer">{cell}</a>;
  return cell;
};
