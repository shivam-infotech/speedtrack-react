...
const DateRangePicker = ({
        ...
        try {
            if (minDate && maxDate) {
                ...
                return Array.from({ length: 24 }, (_, i) =>
                    currentViewDate.add(i - 12, 'month').startOf('month')
                );
            }
        } catch (error) {
    ...
    useEffect(() => {
        if (open) {
            try {
                ...
                let initialStartDate = fromDate ? dayjs(fromDate) : null;
    ...
    const renderDays = (monthStart) => {
        ...
        try {
            ...
            return (
                <Box key={safeFormat(monthStart, 'MM-YYYY')} sx={{
                    width: "100%",
